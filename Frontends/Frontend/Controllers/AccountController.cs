namespace Frontend.Controllers
{
    ...
    using Blaze.DSP.Library.Authenticator;
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            if (TempData.ContainsKey("LoginAuthenticatorViewModel"))
            {
                TempData.Remove("LoginAuthenticatorViewModel");
            }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(user.AuthenticatorSecrete))
                {
                    ModelState.AddModelError("", "Invalid authenticator secrete, contact system administrator.");
                    return View(model);
                }

                TempData.Add("LoginAuthenticatorViewModel", new LoginAuthenticatorViewModel
                {
                    Email = model.Email,
                    Password = model.Password,
                    RememberMe = model.RememberMe,
                    AuthenticatorSecrete = user.AuthenticatorSecrete
                });

                return RedirectToAction("Authenticator", new { returnUrl });
            }
        ...
        [HttpGet]
        [AllowAnonymous]
        public ActionResult Authenticator(string returnUrl)
        {
            if (!TempData.ContainsKey("LoginAuthenticatorViewModel"))
            {
